...
func main() {
	...
	for _, v := range strings.Split(actual, "\n") {
		...
		combined, _ := strconv.Atoi(digits[0] + digits[len(digits)-1])
		part1 += combined
	}
	fmt.Printf("part 1: %v\n", part1)

	// part 2
	var englishDigits = map[string]string{
		"one":   "1",
		"two":   "2",
		"three": "3",
		"four":  "4",
	...
	for _, v := range strings.Split(actual, "\n") {
		var digis []string
		for i, s := range v {
			for eng, digi := range englishDigits {
				if string(s) == digi || strings.HasPrefix(v[i:], eng) {
					digis = append(digis, digi)
				}
			}
		}
		combined, _ := strconv.Atoi(digis[0] + digis[len(digis)-1])
		part2 += combined
	}
	fmt.Printf("part 2: %v\n", part2)
}
...
const actual = `51591twosix4dhsxvgghxq
425nine
llvmhjtr8nbbhrfone
