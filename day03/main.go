...
func main() {
	...
	fmt.Println("part 1", part1(example2))
...
func (s symbol) neighborhood() []symbol {
	return []symbol{
		{row: s.row - 1, col: s.col - 1}, {row: s.row - 1, col: s.col}, {row: s.row - 1, col: s.col + 1},
		{row: s.row, col: s.col - 1}, {row: s.row, col: s.col + 1},
		{row: s.row + 1, col: s.col - 1}, {row: s.row + 1, col: s.col}, {row: s.row + 1, col: s.col + 1},
	}
}
...
func part1(in string) int {
	...
	for i, row := range rows {
		for _, matches := range regexp.MustCompile(`\d+`).FindAllStringIndex(row, -1) {
		...
		for _, matches := range regexp.MustCompile(`[^\d.\n]`).FindAllStringIndex(row, -1) {
	...
	out:
		for _, s := range symbols {
			for _, n := range s.neighborhood() {
				if n.row < 0 || n.row > len(rows) || n.col < 0 || n.col > width {
					fmt.Printf("out %#v \n", n)
					continue
				}
				if p.row == n.row && p.start <= n.col && p.end >= n.col {
					partNo, _ := strconv.Atoi(rows[p.row][p.start:p.end])
					sum += partNo
					break out
				}
			}
